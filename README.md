...
Workflow and Release use the same release-evidence selector so their conclusions stay consistent, but they present different operating views. Workflow monitors the end-to-end lifecycle with live record counts, throughput, stage progress, entry and exit criteria, inputs, outputs, owners, and current blockers. It shows only a compact release-readiness summary. Release owns the full actionable gate review for configuration, delivery quality, in-house work, dataset construction, evaluation, and the final decision, with an evidence explanation and trace link for every gate. `pending` means required evidence is absent; `active` means work is in progress; `blocked` means a gate failed; `complete` means required evidence passed; `aligned` means an artifact references the current requirement; `stale` means it references an older version; and `not required` means an aligned source plan allocates no work to that path. Portfolio health and external engagement facts remain stored inputs; calculated gates are derived from their underlying records.
